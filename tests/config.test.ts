...
describe("scroll config", () => {
  it("defaults preview to enabled", () => {
    const root = mkdtempSync(join(tmpdir(), "scroll-config-"));
  ...
  it("loads user config over defaults", () => {
    ...
    writeFileSync(join(agentDir, "scroll.json"), JSON.stringify({ preview: false }));
  ...
  it("loads project config over user config", () => {
    ...
    writeFileSync(join(agentDir, "scroll.json"), JSON.stringify({ preview: false }));
    writeFileSync(join(cwd, ".pi", "scroll.json"), JSON.stringify({ preview: true }));
